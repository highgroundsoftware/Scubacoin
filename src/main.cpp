...
#if defined(NDEBUG)
# error "Scubacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7296b9ed18ec9d501af2610d79bca8ecc28e9a384a63bb69065831f4c31f4d5c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Scubacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Scubacoin Signed Message:\n";
...
{
    // Scubacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 20000); // Scubacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Scubacoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Scubacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Scubacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Scubacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa3d0971d2b5a111e5221d93103630963bcd0b103ca4ddb00b8cdfa8771f93a72");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Welcome To Scuba COIN";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b56691d94ffc57316c6ad9eaeffa4b160f43aaace567740390e90f48e37f5c30a896e62bf4330501138f608b1b711ede3c709b8092c569daf1e02990bd512277") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517774659;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085057646;

        if (fTestNet)
        {
            block.nTime    = 1517774603;
            block.nNonce   = 385305963;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x16372653e64becc75d4b185a790b0c2d7b5badeaea7cc4d6cd085c0b83520eca"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc4, 0xbe, 0xd4 }; // Scubacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ScubacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ScubacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ScubacoinMiner(CWallet *pwallet)
{
    printf("ScubacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("scubacoin-miner");
    ...
    try { loop {
        ...
        printf("Running ScubacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ScubacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ScubacoinMiner, pwallet));
}
